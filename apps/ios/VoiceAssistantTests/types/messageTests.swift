...
struct MessageTests {
    @Test func testEncodeMessage() async throws {
        let message = Message(role: Role.system, content: [
            ...
            MessageContent(imageUrl: "my-protocol://image-url"),
        ])
        ...
        assert(decodedMessage.content[1].type == .imageUrl && decodedMessage.content[1].imageUrl?.url == "my-protocol://image-url")
