...
class AudioManager {
    ...
    let _updateLabelFunc: (String?, UIColor) -> Void
    
    public init(apiManager: ApiManager, updateLabel: @escaping (String?, UIColor) -> Void) {
        self._apiManager = apiManager
        self._updateLabelFunc = updateLabel
    }
                ...
                case .success(let credentials):
                    ...
                    self.reco.addRecognizedEventHandler() {reco, evt in
                        ...
                        if let message = evt.result.text {
                            self._apiManager.sendChatMessages(message: message) { (result) -> Void in
                                switch result {
                                case .success(let respnoseString):
                                    print("LLM Response: \(respnoseString)")
                                    self._updateLabelFunc(respnoseString, .blue)
                                    Task {
                    ...
                    self.reco.addCanceledEventHandler { reco, evt in
                        ...
                        self._updateLabelFunc(evt.errorDetails?.description, .gray)
                    }
                    ...
                    self._updateLabelFunc("Listening ...", .gray)
                case .failure(let error):
                    print("Error: \(error)")
            }
        }
